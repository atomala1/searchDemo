...
class UserSpecification(private val options: List<String>) : Specification<User> {
    ...
    private fun fieldQueryToCriteria(root: Root<*>, builder: CriteriaBuilder, fq: List<*>): List<Predicate> {
        ...
        fq.asSequence().map { it.toString().split("=".toRegex()) }
                ...
                .forEach { terms ->
                    predicates.addAll(
                            User::class.memberProperties.stream()
                    ...
                    predicates.addAll(
                            User::class.memberProperties.stream()
                                    ...
                                    .map {
                                        val join: ListJoin<User, String> = root.joinList(it.name)
